...
export function parseBarangTerlaris(arrayBuffer) {
  ...
  const colKodeBarang = findCol(headers, ['No. Barang', 'No Barang', 'Kode Barang', 'Kode'])
  ...
  for (const row of rows) {
    ...
    // kodeBarang: kode penuh termasuk varian (misal "1002189.1.01") — dipakai untuk
    // lookup stock per varian secara persis di halaman Produk Terlaris.
    const kodeRaw   = colKodeBarang ? row[colKodeBarang] : null
    const kodeBarang = kodeRaw != null && kodeRaw !== '' ? String(kodeRaw).trim() : null
...
function buildBarangAnalysis(rows) {
  ...
  const byBarang = {}  // { namaBarang: { kuantitas, hargaProduk, kodeBarang } }
  for (const r of rows) {
    ...
    // Pertahankan kodeBarang pertama yang valid — satu namaBarang seharusnya satu kode
  ...
  const rankedBarang = Object.entries(byBarang)
    ...
    .map(([namaBarang, v]) => ({ namaBarang, kodeBarang: v.kodeBarang, kuantitas: v.kuantitas, hargaProduk: v.hargaProduk }))
