...
export function exportBarangTerlaris(rows, { periodLabel, filterDesc } = {}) {
  ...
  // Number format — Stock (col 3), Kuantitas (col 4), Harga Produk (col 5)
  const dataStartRow = aoa.findIndex(r => r[0] === rows[0]?.rank) + 1
  for (let R = dataStartRow; R <= dataStartRow + rows.length - 1; R++) {
    const stockCell = XLSX.utils.encode_cell({ r: R, c: 3 })
    const kuanCell  = XLSX.utils.encode_cell({ r: R, c: 4 })
    const hargaCell = XLSX.utils.encode_cell({ r: R, c: 5 })
  ...
  const totKuanCell  = XLSX.utils.encode_cell({ r: totalRow, c: 4 })
  const totHargaCell = XLSX.utils.encode_cell({ r: totalRow, c: 5 })
