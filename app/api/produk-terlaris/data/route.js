...
import { getLatestProdukAnalysis, getProdukPeriod } from '@/lib/blob'
...
export async function GET(request) {
  ...
  try {
    let result
    if (id) {
      const data = await getProdukPeriod(id)
      if (!data) return NextResponse.json({ error: 'Periode tidak ditemukan.' }, { status: 404 })
      result = { ...data, periodId: id }
    } else {
      result = await getLatestProdukAnalysis()
      if (!result) return NextResponse.json({ error: 'Belum ada data produk terlaris yang diunggah.' }, { status: 404 })
    }
