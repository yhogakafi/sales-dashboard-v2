...
export function exportBarangTerlaris(rows, { periodLabel, filterDesc } = {}) {
  ...
  const aoa = [
    ...
    [],
    ['#', 'Nama Barang', 'Kuantitas', 'Harga Produk'],
    ...rows.map(r => [r.rank, r.namaBarang, r.kuantitas, r.hargaProduk]),
    [],
  ...
  // Number format for Kuantitas (integer) and Harga Produk (currency)
  const range = XLSX.utils.decode_range(ws['!ref'])
  const dataStartRow = aoa.findIndex(r => r[0] === rows[0]?.rank) + 1
  for (let R = dataStartRow; R <= dataStartRow + rows.length - 1; R++) {
    const kuanCell = XLSX.utils.encode_cell({ r: R, c: 2 })
    const hargaCell = XLSX.utils.encode_cell({ r: R, c: 3 })
  ...
  const totKuanCell  = XLSX.utils.encode_cell({ r: totalRow, c: 2 })
  const totHargaCell = XLSX.utils.encode_cell({ r: totalRow, c: 3 })
